...
function StepCTA({ index }) {
  if (index === 0) {
    return (
      <div className="step-cta-group">
        <a href="https://play.google.com/store" target="_blank" rel="noopener noreferrer" className="step-cta-play">
        ...
        <button className="step-cta-qr" onClick={() => { const m = document.getElementById('qr-modal'); if(m) m.style.display = m.style.display === 'flex' ? 'none' : 'flex'; }}>
          {QR_ICON} Xem mã QR
        </button>
      </div>
    );
    ...
    <section className="section">
      <div className="container">
        ...
        <div className="faq-section">
          <h2 className="section-heading" style={{textAlign:blok.heading_align||"center",marginBottom:24,fontSize:blok.heading_size||"24px"}}>{blok.heading}</h2>
